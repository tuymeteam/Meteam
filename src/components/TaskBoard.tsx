    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-start">
      {columns.map((col) => {
        ...
        return (
          <div 
            key={col.status} 
            className={`border border-slate-150 rounded-2xl overflow-hidden ${col.bg} flex flex-col min-h-[450px] shadow-xs`}
          >
            {/* Column Header */}
              ...
              {colTasks.length === 0 && (
                <div className="h-28 border border-dashed border-slate-200 rounded-xl flex items-center justify-center p-4 bg-white/50 text-center select-none text-slate-400">
                  <div className="scale-90">
                    ...
                    <p className="text-[10px] text-slate-400">Không có công việc ở giai đoạn này</p>
                  </div>
                </div>
              )}
            </div>
